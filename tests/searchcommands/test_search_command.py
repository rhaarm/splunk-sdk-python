...
class TestSearchCommand(unittest.TestCase):
    ...
    def test_process(self):
        ...
        self.assertRaises(SystemExit, command.process, ['foo.py', '__GETINFO__', 'undefined_option=value'],
                          output_file=result)
        result.reset()
        observed = result.read()
        self.assertEqual(expected, observed)
        ...
        try:
            command.process(args=['foo.py', '__EXECUTE__', 'undefined_option=value'], input_file=BytesIO('\r\n'),
                            output_file=result)
        ...
        try:
            command.process(args=['foo.py', '__EXECUTE__'], input_file=BytesIO('\r\nAction\r\nraise_error'),
                            output_file=result)
        except SystemExit as e:
            result.reset()
            observed = result.read()
